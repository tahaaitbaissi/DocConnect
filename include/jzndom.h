...
#define JZNDOM_FIELD_USED                   0x2
  ...
  */
  ...
  /*
  ** Position within the object, if known. This is returned by DOM interfaces.
...
** PARAMETERS:
**   jznDomDoc     *jdoc     (IN)  - JSON DOM
**   ub1           *oson_byte(IN)  - OSON byta array[]
**   ub4           oson_byte_len (IN) - OSON byte array length 
**   jsonerr       *ojerr   (OUT) - error when trying to load the oson bytes
...
** NOTES:
*/
#define JsonDomDecodeOSONBytesIntoNode(jdoc, oson_byte,  oson_byte_len, ojerr, out_jdoc) \
  ((jdoc)->mthds_jznDomDoc->jznDomDocDecodeOSONBytesIntoNode(jdoc, oson_byte,  oson_byte_len, ojerr, out_jdoc))
...
*/
sword JsonDomHash(JsonDomDoc *doc, JsonDomNode *node, ub1 *outbuf, ub4 *outlen);
