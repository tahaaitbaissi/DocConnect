...
class MetaData
{
  public :
      ...
      ATTR_DOMAIN_NAME = OCI_ATTR_DOMAIN_NAME,
      ATTR_DOMAIN_SCHEMA = OCI_ATTR_DOMAIN_SCHEMA
     };
...
class   Environment     
{
 public:
  ...
  enum Mode
  {
    ...
    THREADED_MUTEXED_V2 = OCI_THREADED_V2 
  };
  ...
  virtual Connection * createConnection(
                const OCCI_STD_NAMESPACE::string &connectString) = 0;
  virtual Connection * createConnection(const UString &connectString) = 0;
...
class Statement 
{
 public:
  ...
  virtual void setBoolean(unsigned int paramIndex, bool x) = 0;
  ...
  virtual bool getBoolean(unsigned int paramIndex) = 0;
